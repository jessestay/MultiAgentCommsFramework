...
//                 to the addressed agent, defaulting to Exec PM.
//   - Proactive:  dmJesse() lets an agent DM Jesse directly (uses JESSE_SLACK_ID).
//
...
// DM Jesse directly as an agent persona. Uses the known JESSE_SLACK_ID.
// Returns the DM channel ID.
async function dmJesse(client, agent, text) {
  const { JESSE_SLACK_ID } = require('../config');
  if (!JESSE_SLACK_ID) {
    console.log('[dm] JESSE_SLACK_ID not set — skipping proactive DM');
    return null;
  }
