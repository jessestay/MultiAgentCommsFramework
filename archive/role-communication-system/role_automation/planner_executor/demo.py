#!/usr/bin/env python
"""
Demonstration of the Planner-Executor Architecture
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('planner_executor_demo.log')
    ]
)
...
def run_demo():
    """Run the Planner-Executor demonstration."""
    logger.info("Starting Planner-Executor demonstration")
    ...
    # Initialize roles
    es_planner = ExecutiveSecretaryPlanner(storage_manager, message_router)
    md_executor = MarketingDirectorExecutor("MD", storage_manager, message_router)
    smm_executor = SocialMediaManagerExecutor("SMM", storage_manager, message_router)
    ...
    for step in md_tasks.get(plan['id'], []):
        ...
        md_executor.execute_step(plan['id'], step['id'])
    ...
    for step in smm_tasks.get(plan['id'], []):
        ...
        smm_executor.execute_step(plan['id'], step['id'])
